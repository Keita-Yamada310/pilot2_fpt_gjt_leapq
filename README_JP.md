...
現在の項目は、Pilot 2用の32項目です。soundはgetに置き換え、sendは除外しています。
追加比較候補はstart・arrive・askです。v-obj-comp[bare-inf]はlet・see・make・watch、
v-obj-obl[to]はtakeのみ、v-obj-objはgive・show・tell・askです。
see/watch + O + V-ing、start + V-ing、arrive + in NP / arrive homeは、
FPT採点時のnatural alternativeとして別途扱います。
...
8. final CSVにFPT32行・GJT32行が含まれる。
...
## 修正履歴
- FPT練習回答後に停止する問題を修正しました。jsPsych v8で未定義の`acceptable_alternative`を評価していたことが原因です。

- GJTの回答ボタンを「正しい＝左」「間違い＝右」に変更しました。


## スマートフォン表示修正

- GJT英文を原則1行表示にしました。
- 画面幅に収まらない長い英文だけ、13pxを下限として自動的に縮小します。
- 390px以下の小型画面ではカード左右余白も縮小します。
- GJTボタンは左「正しい」、右「間違い」とし、保存されるjudgmentも表示と一致するよう確認済みです。

- iPhone Safariで長いGJT文の自動縮小時に画面が揺れないよう、非表示で1回だけサイズ計算してから表示する方式に変更しました。
