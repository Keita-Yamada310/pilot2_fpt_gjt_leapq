...
// Reduce the font only when a GJT sentence would wrap on a narrow screen.
// The minimum is kept at 15px so the item remains readable on small phones.
function fitGjtSentenceToOneLine() {
  const el = document.querySelector(".gjt-sentence");
  if (!el) return;

  el.classList.add("gjt-font-fitted");
  el.style.fontSize = "";
...
function makeGjtTrial(isPractice = false) {
  return {
    ...
    on_load: function() {
      fitGjtSentenceToOneLine();
      window.setTimeout(fitGjtSentenceToOneLine, 50);
    },
